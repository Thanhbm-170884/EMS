                            ...
                            <c:forEach var="shift" items="${shifts}">
                                <tr class="${!shift.working ? 'inactive-row' : ''}">
                                    <td>
                                        <span class="day-name">
                                            <c:choose>
                                                <c:when test="${shift.dayOfWeek==2}">
                                                    Thứ Hai
                                                </c:when>
                                                <c:when test="${shift.dayOfWeek==3}">
                                                    Thứ Ba
                                                </c:when>
                                                <c:when test="${shift.dayOfWeek==4}">
                                                    Thứ Tư
                                                </c:when>
                                                <c:when test="${shift.dayOfWeek==5}">
                                                    Thứ Năm
                                                </c:when>
                                                <c:when test="${shift.dayOfWeek==6}">
                                                    Thứ Sáu
                                                </c:when>
                                                <c:when test="${shift.dayOfWeek==7}">
                                                    Thứ Bảy
                                                </c:when>
                                                <c:when test="${shift.dayOfWeek==1}">
                                                    Chủ Nhật
                                                </c:when>
                                            </c:choose>
                                        </span>
...
<script>
    /* Dữ liệu từ server */
    const _today = '${today}';
    <%-- Nếu vừa submit lỗi (formShifts có dữ liệu) → ưu tiên dựng lại form
         từ đúng giá trị user vừa gõ, thay vì nạp lại lịch hiện có trong DB. --%>
    <c:set var="bridgeShifts" value="${not empty formShifts ? formShifts : shifts}" />
    const _hasSchedule = ${ (not empty formShifts) or hasSchedule };
    const _existingShifts = [
        <c:forEach var="s" items="${bridgeShifts}" varStatus="st">
        {
            dow:     ${s.dayOfWeek},
            working: ${s.working},
...
<script>
    ...
    function renderConflicts() {
        ...
        const dowCount = {};
        rules.forEach(rule => rule.days.forEach(dow => {
    ...
    /* Có lỗi validate từ server (doPost) → mở lại modal và hiển thị
       đúng khối validation-error đang dùng cho validate phía client */
    openModal('${hasSchedule ? "edit" : "add"}');
    showValidation("${fn:escapeXml(error)}");
    </c:if>
</script>
</body>
